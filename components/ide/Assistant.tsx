...
export default function AssistantPanel({ openFile, onClose, visible, avatar = '>_' }: AssistantPanelProps) {
  ...
  return (
    ...
    >
      <Header T={T} count={messages.filter((m) => m.role !== 'user' || true).length} onClose={onClose} />
      ...
      >
        ...
        {messages.length <= 1 && <Starters T={T} disabled={streaming} onPick={(q) => send(q)} />}
      </div>
...
// ── Pieces ──────────────────────────────────────────────────────────
function Header({ T, count, onClose }: { T: Theme; count: number; onClose: () => void }) {
  return (
    <div
      style={{
        padding: '10px 14px',
          ...
          <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {m.followups.map((q, i) => (
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = T.chrome.selBg;
                }}
                onMouseLeave={(e) => {
...
function Starters({ T, onPick, disabled }: { T: Theme; onPick: (q: string) => void; disabled: boolean }) {
  const items = [
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {items.map((q) => (
          <button
            ...
            onMouseEnter={(e) => {
              if (!disabled) e.currentTarget.style.background = T.chrome.selBg;
            }}
            onMouseLeave={(e) => {
              if (!disabled) e.currentTarget.style.background = T.chrome.hoverBg;
            }}
          >
            {q}
          </button>
...
function splitCodeFences(body: string) {
...
function renderChatInline(
  text: string,
  citations: string[],
  openFile: ((p: string) => void) | undefined,
  T: Theme
): React.ReactNode[] {
  ...
  while (i < text.length) {
    ...
    if ((m = rest.match(/^\*\*([^*]+)\*\*/))) {
      out.push(
        <strong key={key++} style={{ fontWeight: 700, color: T.chrome.fg }}>
          {m[1]}
        </strong>
      );
      i += m[0].length;
    ...
    if ((m = rest.match(/^\*([^*\s][^*]*?)\*/))) {
      out.push(
        <em key={key++} style={{ color: T.syntax.emItalic, fontStyle: 'italic' }}>
          {m[1]}
        </em>
      );
      i += m[0].length;
    ...
    if (typeof last === 'string') out[out.length - 1] = (last as string) + text[i];
...
function ChatCodeBlock({ body, lang, T }: { body: string; lang?: string; T: Theme }) {
  const tokens = highlight(lang || 'md', body);
  return (
    <pre
      style={{
        margin: '10px 0',
          ...
          {toks.map((tok, j) => (
            <span
              ...
              style={{
                ...
                color: (T.syntax as any)[tok.t] || T.syntax.text,
                fontWeight: tok.t === 'bold' ? 700 : 400,
                fontStyle: tok.t === 'emItalic' ? 'italic' : 'normal',
              }}
            >
              {tok.v}
            </span>
...
export function FilePreviewCard({
  ...
  const body = PORTFOLIO_FS.files[path] || '';
  const lines = body.split('\n');
  ...
  return (
    ...
    >
      ...
      {firstHeading && (
        <div style={{ fontSize: 13, fontWeight: 600, color: T.chrome.fg, marginBottom: 2 }}>{firstHeading}</div>
      )}
      {snippet && (
        <div style={{ fontSize: 11.5, color: T.chrome.fgDim, lineHeight: 1.45 }}>
          {snippet}
          {snippet.length >= 100 ? '…' : ''}
        </div>
      )}
...
export function StackPills({ items, T }: { items: string[]; T: Theme }) {
  return (
      ...
      >
        {table.headers.map((h, i) => (
          <div
            ...
            style={{
              padding: '6px 10px',
              borderRight: i < table.headers.length - 1 ? '1px solid ' + T.chrome.border : 0,
            }}
          >
            {h}
          </div>
    ...
    <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
      {actions.map((a, i) => (
        <a
          key={i}
          href={a.do}
          target={a.do && a.do.startsWith('http') ? '_blank' : undefined}
          rel="noreferrer"
          style={{
            display: 'inline-flex',
            alignItems: 'center',
...
const INTRO_MESSAGE: ChatMessage = {
  ...
  streaming: false,
  citations: [],
  content: `Hi — I'm grounded in this repo's files. Ask about Saurabh's work, stack, availability, or any specific project.
