...
import type { Theme, Platform } from '@/types/ide';

type BootLineKind = 'sect' | 'ok' | 'wait' | 'warn' | 'done';
interface BootLine {
  delay: number;
  kind: BootLineKind;
  text: string;
...
function bootLinesFor(_platform: Platform): BootLine[] {
  return [
...
export default function Splash() {
  ...
  const [phase, setPhase] = useState<'boot' | 'flash' | 'fade' | 'gone'>('boot');
  ...
  useEffect(() => {
    ...
    return () =>
      timersRef.current.forEach((t) => (t && typeof t === 'object' && 'clear' in t ? t.clear() : clearTimeout(t)));
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 20, color: T.chrome.fg, fontWeight: 600, letterSpacing: '-0.005em' }}>
            saurabhjalendra.com
          </div>
              ...
              line.kind === 'done'
                ...
                : line.kind === 'warn'
                  ...
                  : line.kind === 'ok'
                    ...
                    : T.chrome.fgFainter;
            const tick =
              line.kind === 'done' ? '↵' : line.kind === 'warn' ? '!' : line.kind === 'wait' ? '·' : '✓';
            return (
              <div
                key={i}
                style={{
        ...
        >
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <span key={tipIdx} style={{ animation: 'sj-tip-in .26s ease-out' }}>
              {tips[tipIdx]}
            </span>
          </span>
          <span>
            <kbd style={kbdStyle(T)}>esc</kbd> skip ·{' '}
            <span style={{ color: T.chrome.fg }}>{Math.round(progress)}%</span>
          </span>
        </div>
      </div>
