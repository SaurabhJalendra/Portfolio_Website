...
declare global {
  interface Window {
    IDE_TOAST?: (text: string, kind?: ToastKind, opts?: { ttl?: number }) => void;
  }
}
...
function Toast({ toast, T }: { toast: ToastEntry; T: Theme }) {
  const accent =
    toast.kind === 'ok'
      ? '#7be39a'
      : toast.kind === 'err'
