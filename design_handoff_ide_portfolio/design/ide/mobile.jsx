// Mobile view — replaces the IDE shell entirely below 760px.
//
...
(function () {
  ...
  // Hook: returns whether viewport width is below `bp` (default 760).
  ...
  function MobileApp({ tweaks, setTweak }) {
    ...
    return (
      <div style={{
        ...
        background: T.chrome.editor, color: T.chrome.fg,
        ...
        <Section T={T} title="About" id="about">
          <RenderedMarkdown body={P.files['about.md']} T={T} onLink={setSheetFile}/>
        </Section>
        <Section T={T} title="Selected work" id="projects">
          <Projects T={T} P={P} onOpen={setSheetFile}/>
        </Section>
        <Section T={T} title="Writing" id="writing">
          <WritingList T={T} P={P} onOpen={setSheetFile}/>
        </Section>
        <Section T={T} title="Now" id="now" subtitle="week of May 19, 2026">
          <RenderedMarkdown body={P.files['now.md']} T={T} onLink={setSheetFile}/>
        </Section>
  ...
  // ------------------------------------------------------------------ Top bar
  function TopBar({ T, tweaks, setTweak }) {
    ...
    return (
      <div style={{
        ...
        <span style={{fontSize: 13, color: T.chrome.fg, fontWeight: 600, letterSpacing: '-0.005em'}}>
          saurabhjalendra<span style={{color: T.accent}}>.com</span>
        </span>
        <button onClick={cycle} title={'Theme: ' + tweaks.theme} style={{
          display:'flex', alignItems:'center', gap:8,
          background: 'transparent', border: '1px solid ' + T.chrome.border,
          borderRadius: 8, padding: '4px 8px 4px 6px',
          color: T.chrome.fgDim, cursor: 'pointer',
    ...
    return (
      <div style={{padding: '36px 24px 28px'}}>
        <div style={{
          ...
          <span style={{width:6, height:6, borderRadius:'50%', background:'#7be39a', boxShadow:'0 0 8px #7be39a'}}/>
          Available — Q3 2026
        </div>
        <h1 style={{
          fontSize: 38, lineHeight: 1.1, margin: 0,
          color: T.chrome.fg, fontWeight: 700,
          letterSpacing: '-0.018em',
        }}>Saurabh Jalendra</h1>
        <div style={{
          marginTop: 6, color: T.chrome.fgDim, fontSize: 14,
          fontFamily:'"Geist Mono",monospace',
        }}>[Engineer · Product Builder] · IST</div>
        <p style={{
          marginTop: 18, fontSize: 17, lineHeight: 1.55,
          color: T.chrome.fg,
        }}>
          [One-line tagline — what you make and for whom.] I build products end-to-end
          and care about the boring foundations that make ambitious work possible.
        </p>
      </div>
    );
  }
  ...
  // ------------------------------------------------------------------ Section
  function Section({ T, title, subtitle, id, children }) {
    return (
      <section id={id} style={{
        ...
        <div style={{
          ...
          <h2 style={{
            margin: 0, fontSize: 20, fontWeight: 700, color: T.chrome.fg,
            letterSpacing: '-0.005em',
          }}>{title}</h2>
          {subtitle && <span style={{
            fontSize: 11, color: T.chrome.fgFainter,
            fontFamily:'"Geist Mono",monospace',
          ...
          >
            <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', gap: 8, marginBottom: 6}}>
              ...
              <span style={{
                ...
                color: c.status === 'wip' ? '#ffd07a' : c.status === 'archived' ? T.chrome.fgFainter : '#7be39a',
              }}>{c.status} {c.year && '· ' + c.year}</span>
            </div>
  ...
  // ------------------------------------------------------------------ Writing
  function WritingList({ T, P, onOpen }) {
    const dir = P.tree.find(n => n.path === 'writing');
    if (!dir) return null;
    return (
      <div style={{display:'grid', gap: 1, background: T.chrome.border, borderRadius: 10, overflow: 'hidden', border: '1px solid ' + T.chrome.border}}>
        {dir.children.map(c => {
          ...
          return (
            <button key={c.path} onClick={() => onOpen(c.path)} style={{
              display:'flex', alignItems:'baseline', justifyContent:'space-between',
              gap: 10, padding: '14px 16px',
              background: T.chrome.titlebar,
              border: 0, color: T.chrome.fg,
  ...
  function ContactCard({ T }) {
    ...
    return (
      <div style={{display:'grid', gap: 1, background: T.chrome.border, borderRadius: 10, overflow: 'hidden', border: '1px solid ' + T.chrome.border}}>
        {rows.map(r => (
          <a key={r.label} href={r.href || undefined} target={r.href && r.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer"
            style={{
              display: 'flex', alignItems:'center', justifyContent:'space-between',
              padding: '14px 16px', background: T.chrome.titlebar,
              color: T.chrome.fg, textDecoration: 'none',
              fontFamily:'"Geist Mono",monospace',
              cursor: r.href ? 'pointer' : 'default',
            }}>
            <span style={{fontSize: 11.5, color: T.chrome.fgFainter, letterSpacing: '0.10em', textTransform: 'uppercase'}}>{r.label}</span>
  ...
  // ------------------------------------------------------------------ FAB
  function AskFab({ T, onClick }) {
    return (
      <button onClick={onClick}
        style={{
          position: 'fixed', right: 18, bottom: 22, zIndex: 90,
  ...
  function FileSheet({ T, file, onClose }) {
    const body = window.PORTFOLIO_FS.files[file] || '(file not found)';
    return (
        ...
        <div onClick={(e) => e.stopPropagation()} style={{
          ...
          <div style={{
            ...
            <button onClick={onClose} style={{
              background: 'transparent', border: 0, color: T.chrome.fg,
          ...
          <div style={{flex: 1, overflow:'auto'}}>
            <window.MarkdownPreview body={body} openFile={() => {}}/>
          </div>
        </div>
        <style>{`
  ...
  function RenderedMarkdown({ body, T, onLink }) {
    return (
      <div style={{margin: '0 -8px'}}>
        <window.MarkdownPreview body={body} openFile={onLink}/>
      </div>
    );
  }
  ...
  function AskSheet({ T, onClose, onOpenFile }) {
    const [messages, setMessages] = useState([
      { role:'assistant', content: "Hi — I'm grounded in this repo's files. Ask about Saurabh's work, stack, or availability.", citations: [], followups: ['What\'s his stack?', 'Show me his strongest project', 'Is he available?'] },
    ]);
    ...
    async function send(text) {
      ...
      setStreaming(true);
      // Reuse the desktop chat() function (defined globally inside assistant.jsx scope —
      // since both run in browser, we look it up via a side door: window.chat may not be
      // set, so we call the mock directly through the public hook we DO export).
      // For independence on mobile, we duplicate mockReply via window.assistantMockReply.
      const reply = window.__mobileMockReply(q);
      setMessages(m => [...m, { role:'assistant', content: '', citations: reply.citations, followups: reply.followups, streaming: true }]);
      // Simple streaming
      ...
      <div onClick={onClose} style={{
        ...
        <div onClick={(e) => e.stopPropagation()} style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{fontFamily:'"Geist Mono",monospace', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: T.chrome.fgFainter}}>Assistant</div>
              <div style={{display:'flex', alignItems:'center', gap: 6, fontSize: 12, color: T.chrome.fg, marginTop: 2}}>
                <span style={{width: 7, height: 7, borderRadius: '50%', background: T.accent, boxShadow: '0 0 6px ' + T.accent}}/>
          ...
          <div ref={bodyRef} style={{flex:1, overflow:'auto', padding:'14px', display:'flex', flexDirection:'column', gap: 14}}>
            {messages.map((m, i) => (
              <ChatBubble key={i} m={m} T={T} onOpenFile={onOpenFile} onFollowup={send}/>
            ))}
          </div>

          <div style={{padding:'10px 12px 14px', borderTop: '1px solid ' + T.chrome.border, background: T.chrome.sidebar}}>
            <div style={{display:'flex', gap: 8, alignItems:'flex-end',
              background: T.chrome.editor, borderRadius: 12,
              border: '1px solid ' + T.chrome.border, padding:'10px 12px',
            }}>
              <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={onKey}
                ...
                style={{
                  ...
                  color: T.chrome.fg, fontFamily:'inherit', fontSize: 14, lineHeight: 1.45,
              ...
              <button onClick={() => send()} style={{
                ...
                <svg width="16" height="16" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M7 11V3 M3 7l4-4 4 4"/></svg>
              </button>
            </div>
          </div>
        </div>
  ...
  function ChatBubble({ m, T, onOpenFile, onFollowup }) {
    if (m.role === 'user') {
      return (
        <div style={{
          alignSelf:'flex-end', maxWidth: '88%',
          background: T.chrome.selBg, color: T.chrome.fg,
          border: '1px solid ' + T.chrome.border,
      ...
      <div style={{display:'flex', gap: 10, alignItems:'flex-start'}}>
        ...
        <div style={{flex:1, minWidth: 0}}>
          {isThinking ? (
            ...
            <window.ChatMarkdown body={m.content} citations={m.citations || []} openFile={onOpenFile} streaming={m.streaming} T={T}/>
          )}
          {m.stack && m.stack.length > 0 && !m.streaming && window.StackPills && (
            <window.StackPills items={m.stack} T={T}/>
          )}
          {m.table && !m.streaming && window.ComparisonTable && (
            <window.ComparisonTable table={m.table} T={T}/>
          )}
          {m.actions && m.actions.length > 0 && !m.streaming && window.ActionButtons && (
            <window.ActionButtons actions={m.actions} T={T}/>
          )}
          {m.citations && m.citations.length > 0 && !m.streaming && window.FilePreviewCard && (
            <div style={{display:'flex', flexDirection:'column', gap: 6, marginTop: 10}}>
              ...
              {m.citations.map(c => (
                <window.FilePreviewCard key={c} path={c} T={T} onOpen={onOpenFile}/>
              ))}
            </div>
          )}
