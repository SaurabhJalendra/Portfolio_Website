...
//
// Usage from anywhere:
//   window.IDE_FLY_TO_TAB(chipElement, 'projects/alpha.md', openFile);
//
// Falls back to a plain openFile if measurement fails.

(function () {
  async function flyToTab(chipEl, path, openFile) {
    if (!chipEl || !openFile) {
      openFile?.(path);
      return;
    }
    ...
    const text = chipEl.innerText || path;
    ...
    const tabEl = document.querySelector('[data-tab-path="' + cssEscape(path) + '"]');
  ...
  // Ensure the keyframe for the landing-flash exists once.
  if (typeof document !== 'undefined' && !document.getElementById('sj-fly-css')) {
