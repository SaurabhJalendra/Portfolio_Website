...
const { useEffect: useEffectP, useRef: useRefP, useState: useStateP, useContext: useCtxP, useMemo: useMemoP } = React;
...
function fuzzyScore(needle, hay) {
  ...
  // subseq
  let hi = 0, score = 200, lastMatch = -1;
...
function PaletteShell({ q, setQ, items, onPick, footer, placeholder, leadingIcon }) {
  const T = useCtxP(window.ThemeCtx);
  const [sel, setSel] = useStateP(0);
  const listRef = useRefP(null);
  useEffectP(() => setSel(0), [items.length, q]);
  ...
  return (
    <div style={{
      ...
      width: 640, maxHeight: 460, display:'flex', flexDirection:'column',
      background: T.chrome.editor, color: T.chrome.fg,
      borderRadius: 12,
      boxShadow:'0 30px 80px rgba(0,0,0,0.55), 0 0 0 1px '+T.chrome.borderStrong,
      overflow:'hidden', zIndex: 1000,
    }}>
      <div style={{display:'flex', alignItems:'center', gap:10, padding:'14px 18px', borderBottom:'1px solid '+T.chrome.border}}>
        <span style={{color:T.chrome.fgDim, fontFamily:'"Geist Mono",monospace', fontSize:13}}>{leadingIcon}</span>
        <input
          autoFocus
          value={q}
          ...
          style={{
            ...
            fontSize:15, color:T.chrome.fg, fontFamily:'"IBM Plex Mono",monospace',
          }}
        />
        ...
        {items.map((it, i) => (
          ...
          >
            ...
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:13.5, color:T.chrome.fg, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{it.title}</div>
              {it.meta && <div style={{fontSize:11.5, color:T.chrome.fgFainter, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontFamily:'"Geist Mono",monospace'}}>{it.meta}</div>}
            </div>
      ...
      <div style={{display:'flex', justifyContent:'space-between', padding:'10px 16px', borderTop:'1px solid '+T.chrome.border, fontSize:11.5, color:T.chrome.fgFainter, fontFamily:'"Geist Mono",monospace'}}>
        <span>{footer}</span>
        <span><kbd style={kbdStyle(T)}>↑</kbd><kbd style={kbdStyle(T)}>↓</kbd> navigate &nbsp; <kbd style={kbdStyle(T)}>↵</kbd> select</span>
      </div>
    </div>
  );
}
...
function kbdStyle(T) {
  return {
    ...
    minWidth:22, height:22, padding:'0 6px', borderRadius:5, margin:'0 2px',
...
function QuickOpen({ open, onClose, openFile }) {
  const T = useCtxP(window.ThemeCtx);
  const [q, setQ] = useStateP('');
  const allFiles = useMemoP(() => {
    const out = [];
    const walk = (ns) => ns.forEach(n => { if (n.type === 'file') out.push(n.path); if (n.children) walk(n.children); });
    walk(window.PORTFOLIO_FS.tree);
    return out;
  }, []);

  const items = useMemoP(() => {
    const scored = allFiles.map(p => ({ p, s: fuzzyScore(q, p) }))
      .filter(x => x.s >= 0)
      .sort((a, b) => b.s - a.s)
      .slice(0, 20)
      .map(({ p }) => ({
  ...
  return (
    <PaletteShell
      q={q} setQ={setQ} items={items}
      onPick={(it) => { it.action(); onClose(); }}
...
function CommandPalette({ open, onClose, openFile, setTheme, themeName, setTerminalVisible, terminalVisible, setMotion, motion, setShowMinimap, showMinimap, setAssistantVisible, assistantVisible, focusFile }) {
  const T = useCtxP(window.ThemeCtx);
  const platform = useCtxP(window.PlatformCtx);
  ...
  ], [themeName, terminalVisible, motion, showMinimap, assistantVisible, openFile, setTheme, setTerminalVisible, setShowMinimap, setMotion, setAssistantVisible]);

  const items = useMemoP(() => (
    cmds.map(c => ({ c, s: fuzzyScore(q, c.title + ' ' + (c.meta||'')) }))
      .filter(x => x.s >= 0)
      .sort((a, b) => b.s - a.s)
      .map(({ c }) => c)
  ), [q, cmds]);
...
function PaletteOverlay({ kind, onClose, ...props }) {
  const T = useCtxP(window.ThemeCtx);
  if (!kind) return null;
  return (
    <div
    ...
    >
      <div onClick={(e) => e.stopPropagation()}>
        {kind === 'quickopen' && <QuickOpen open onClose={onClose} {...props}/>}
...
window.IDEPalette = PaletteOverlay;
