...
//
// Exports:  window.highlight(lang, body) -> [[ {t: tokenType, v: string} ]]
//             outer array = lines; inner array = tokens within the line.
//             token types are mapped to colors in editor.jsx via theme.

(function () {
  function tk(t, v) { return { t, v }; }
  ...
  function hl_md(line) {
    ...
    if (/^\s*-\s/.test(line)) {
      ...
      return [tk('punct', m[1]), tk('listBullet', '- '), ...inline(m[3])];
    }
  ...
  const PER_LANG = { md: hl_md, json: hl_json, yaml: hl_yaml, ini: hl_ini };

  window.highlight = function highlight(lang, body) {
    const fn = PER_LANG[lang] || ((l) => [tk('text', l)]);
    return body.split('\n').map(fn);
  };

  // Headings → outline. Returns [{depth, text, line}]
  window.extractOutline = function (body) {
    const out = [];
    body.split('\n').forEach((line, i) => {
